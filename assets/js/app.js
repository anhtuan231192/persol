...
function checkUserLogin() {
    ...
    if (token) {
        ...
        try {
            ...
            if (userWelcome) userWelcome.textContent = `Welcome, ${payload.data.email}`; // Changed
...
async function initHomePage() {
    ...
    if (!newArrivalsContainer || !productsContainer || !template) {
        console.warn('Missing element for homepage or missing #product-card-template');
        return;
    }
    ...
    const renderGrid = (container, productList) => {
        ...
        productList.forEach(product => {
            ...
            if (priceEl) priceEl.textContent = `${parseFloat(product.Price).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`; // Changed currency format

            if(imgEl){
                imgEl.src = product.main_image ? `uploads/product_images/${product.main_image}` : 'uploads/product_images/placeholder.jpg';
...
async function initProductsPage() {
    ...
    categoryFilter.innerHTML = '<option value="">-- Filter by category --</option>'; // Changed
...
async function renderProductList() {
    ...
    const productsRes = await api.getProducts({ brand_id, category_id, min_price, max_price }); // Pass prices
    container.innerHTML = '';
...
async function initProductDetailPage(id) {
    if (!id) {
         mainContent.innerHTML = '<h1>Invalid Product ID</h1><a href="#products">&larr; Back to list</a>'; // Changed
         return;
    }
    ...
    if (!productRes || !productRes.success || !productRes.data) {
        ...
        console.error("Failed to load product details:", productRes ? productRes.message : "No response");
        return;
    }
    ...
    if (productCategoryEl) productCategoryEl.textContent = product.category_name || 'N/A';
    if (productPriceEl) productPriceEl.textContent = `${parseFloat(product.Price).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`; // Changed currency format
    if (productDescriptionEl) productDescriptionEl.textContent = product.product_description;

    if (productImageEl) {
        productImageEl.src = product.main_image ? `uploads/product_images/${product.main_image}` : `uploads/product_images/placeholder.jpg`;
    ...
    if (specsBody) {
        ...
        if (product.specifications && product.specifications.length > 0) {
            product.specifications.forEach(spec => {
                specsBody.innerHTML += `<tr><td>${spec.spec_name}</td><td>${spec.spec_value}</td></tr>`;
            });
        } else {
             specsBody.innerHTML = '<tr><td colspan="2">No specifications available.</td></tr>'; // Changed
        }
    }
...
async function initComparePage() {
    ...
    if (compareList.length === 0) {
        container.innerHTML = '<p>You haven\'t added any products to the comparison list.</p><a href="#products">View products</a>'; // Changed
        return;
    }
    ...
    products.forEach(p => {
        if (Array.isArray(p.specifications)) {
            p.specifications.forEach(s => allSpecNames.add(s.spec_name));
        }
    });
    ...
    products.forEach(p => {
        table += `<th>
                    <img src="uploads/product_images/${p.main_image || 'placeholder.jpg'}" alt="${p.product_name || ''}">
    ...
    allSpecNames.forEach(specName => {
        table += `<tr><td>${specName}</td>`;
        products.forEach(p => {
            let specValue = 'N/A';
            if (Array.isArray(p.specifications)) {
                 const spec = p.specifications.find(s => s.spec_name === specName);
                 if (spec) specValue = spec.spec_value;
            }
...
async function initContactPage() {
    const mapElement = document.getElementById('map-placeholder');
     if (mapElement) {
         mapElement.textContent = "(Google Maps will display here if you configure an API Key)"; // Changed
     }
...
async function initCheckoutPage() {
    ...
    if (cart.length === 0) {
        mainContent.innerHTML = '<h2>Cart is empty!</h2><a href="#products">Continue shopping</a>'; // Changed
        return;
    }
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if(res.success) {
            alert(`Order placed successfully! Your order code is: ${res.order_code || 'N/A'}`); // Changed
            cart = [];
...
async function initProfilePage() {
    ...
    tableBody.innerHTML = '<tr><td colspan="5">Loading order history...</td></tr>'; // Changed
    ...
    if (res.success && res.data && res.data.length > 0) {
        res.data.forEach(order => {
            const orderDate = order.created_at ? new Date(order.created_at).toLocaleDateString('en-CA') : 'N/A';
            const totalAmount = order.total_amount ? parseFloat(order.total_amount).toLocaleString('en-US', { style: 'currency', currency: 'USD' }) : '$0.00';
            tableBody.innerHTML += `
                <tr>
...
async function loadVisitorCount() {
    ...
    try {
        ...
        if (res && res.success && res.visitor_count !== undefined) {
            ...
            console.warn("Failed to get visitor count:", res ? res.message : "No response");
        }
    } catch (e) {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    const currentTimeText = currentTextParts.length >= 2 ? currentTextParts[0] + '|' + currentTextParts[1] : `Today: ${date} | Time: ${time}`;
...
function router() {
    ...
    const allowedPages = ['home', 'products', 'product', 'contact', 'compare', 'login', 'register', 'checkout', 'profile'];
